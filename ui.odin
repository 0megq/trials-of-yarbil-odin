	...
	if field.selected {
		for key := rl.GetKeyPressed(); key != .KEY_NULL; key = rl.GetKeyPressed() {
			add_char :: proc(field: ^NumberField, input_char: string) {
				start := "" if field.cursor_pos == 0  else strings.cut(field.current_string, 0, int(field.cursor_pos))
				end := strings.cut(field.current_string, int(field.cursor_pos), len(field.current_string) - int(field.cursor_pos))
				delete(field.current_string, context.allocator)
			...
			case .BACKSPACE:
				if field.cursor_pos > 0 {
					...
					start := "" if field.cursor_pos == 1 else strings.cut(field.current_string, 0, int(field.cursor_pos) - 1)
					end := strings.cut(field.current_string, int(field.cursor_pos), len(field.current_string) - int(field.cursor_pos))
					delete(field.current_string, context.allocator)
			...
			case .DELETE:
				if field.cursor_pos < i32(len(field.current_string)) {
					...
					start := "" if field.cursor_pos == 0 else strings.cut(field.current_string, 0, int(field.cursor_pos))
					end := strings.cut(field.current_string, int(field.cursor_pos) + 1, len(field.current_string) - int(field.cursor_pos))
					delete(field.current_string, context.allocator)
					field.current_string = strings.join({start, end}, "")
				}
			case .LEFT:
...
draw_number_field :: proc(field: NumberField) {
	...
	label_text := strings.clone_to_cstring(field.label, context.temp_allocator)
	label_width := f32(rl.MeasureText(label_text, NUMBER_FIELD_FONT_SIZE))
	label_pos: Vec2i = {i32(field.rect.x), i32(field.rect.y + field.rect.height / 2) - NUMBER_FIELD_FONT_SIZE / 2}
	rl.DrawRectangle(label_pos.x, i32(field.rect.y), i32(label_width), i32(field.rect.height), field.normal_color)
	...
	if field.selected && show_text_cursor {
		...
		rl.DrawLineV({line_x, text_pos.y + NUMBER_FIELD_FONT_SIZE}, {line_x + avg_character_width, text_pos.y + NUMBER_FIELD_FONT_SIZE}, rl.BLACK)
	}
}
